...
flowchart TD
    subgraph User [Người dùng]
        A(1. Nhập từ khóa Tìm kiếm)
        C(3. Click icon Sửa trên dòng sản phẩm)
        D(5. Nhập thông tin cập nhật)
        E(6. Click nút Lưu)
    end

    subgraph System [Hệ thống]
        B(2. Lọc dữ liệu & Mở rộng nhóm khớp)
        S1(4. Mở Modal Chỉnh sửa với dữ liệu tương ứng)
        S2{7. Xác thực dữ liệu}
        S3(7a. Hiển thị thông báo lỗi)
        S4(8. Cập nhật DB & Ghi Log Chatter)
        S5(9. Tải lại danh sách & Đóng Modal)
    end
